...
**Versión:** 1.0  ·  **Fecha:** 4 de mayo de 2026  ·  **URL del sistema:** https://naves-frontend.huem98.easypanel.host

NAVES (New Business Adventures) es la plataforma de **gestión del trabajo de grado del MBA de INALDE Business School**. Permite a los participantes documentar su anteproyecto, a los profesores revisar y asignarse a equipos, y al super administrador orquestar todo el ciclo (cohortes, fechas, sábana de proyectos, comunicación).

Este manual cubre los **3 roles** del sistema:

| Rol | Cómo entra | Qué puede hacer |
|---|---|---|
| **Super administrador** | Tab *Profesor* + email + clave | Todo: cohortes, participantes, profesores, anteproyectos, sábana, solicitudes, auditoría |
...
3. [Manual del Super Administrador](#3-manual-del-super-administrador)
...
## 3. Manual del Super Administrador

### 3.1 Dashboard general

Al entrar como super administrador, ves una sola opción que conduce al panel completo:

![Dashboard del super administrador](screenshots/03-dashboard-admin.png)

Tu rol aparece marcado como **`SUPER_ADMIN`** en color rojo, bajo el email de la sesión.

### 3.2 Panel administrativo

El panel agrupa **7 módulos independientes**, cada uno con su acción específica:
...
Lista de profesores activos del programa. Distingue **Super Admin** (rojo) de **Profesor** (gris).
...
- Checkbox **"Es super admin"** — marca solo si quieres que el nuevo profesor también administre el sistema

Al crear, queda registrado en la lista:

![Profesor creado](screenshots/10-admin-profesores-list.png)

**Editar:** click "Editar" en cualquier fila para cambiar nombre, áreas, activar/desactivar o promover a super admin.
...
- **Actor** — tipo (sistema/participante/profesor/super_admin) + UUID truncado
...
Cuando entras como profesor regular (sin permisos de super admin), tu dashboard muestra **3 atajos** directos a tus tareas principales:
...
Mismo listado y detalle que ve el super admin (sección 3.6), filtrado por cohortes asignadas a ti. Puedes leer cualquier anteproyecto enviado y descargar su PDF.

### 4.2 Sábana

Lectura de la sábana consolidada (sección 3.7). Puedes consultar, **descargar el PDF**, ver asignaciones, pero **no editas** (la asignación final la guarda el super admin).

### 4.3 Solicitudes

Idéntico al panel del super admin (sección 3.8). Puedes aprobar o rechazar solicitudes de desarchivado de los equipos asignados a ti.

> **Diferencia con el super admin:** no ves los módulos de Cohortes, Cargar participantes, Profesores ni Auditoría. Esos son responsabilidad del super administrador del programa.
...
**Si aún no tienes equipo:** te aparecerá un botón **"Crear mi equipo"** con campo opcional para el nombre.

**Si ya tienes equipo (caso "Test Team"):**
...
- Para agregar miembros: usa el buscador por nombre → click **"Buscar"** → selecciona y click **"Agregar"**.

![Buscar miembros](screenshots/20-participante-equipo-buscar.png)

> El buscador solo muestra participantes de **tu misma cohorte** que aún **no están en otro equipo**.
...
![Selector CIIU](screenshots/22-participante-ciiu-picker.png)
  ...
  - **Canvas del negocio** (8 campos): cliente y problema, canales, relaciones, ingresos, recursos, actividades, socios, costos.
  - **Validación del mercado**: fuentes primarias y secundarias.
  - **Cronograma** — entre **5 y 10 hitos**, cada uno con descripción, fecha de inicio y fin.
...
**Si aún no hay asignación:** mensaje informativo. La asignación ocurre cuando el super admin completa la sábana de proyectos y la marca como "comunicada".
...
| **Super admin** | Profesor con permisos extendidos para administrar todo el sistema |
